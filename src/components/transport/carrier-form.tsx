...
import { createCarrier, updateCarrier } from '@/actions/transport.actions'
...
import { type CarrierInput, carrierSchema } from '@/validations/transport'
...
export function CarrierForm({
  ...
  carrierId?: string
  defaultValues?: Partial<CarrierInput>
  ...
  async function onSubmit(values: CarrierInput) {
    const result = carrierId ? await updateCarrier(carrierId, values) : await createCarrier(values)

    if (!result.ok) {
      if (result.fieldErrors) {
        for (const [field, messages] of Object.entries(result.fieldErrors)) {
          if (messages?.[0]) setError(field as keyof CarrierInput, { message: messages[0] })
        }
      }
    ...
    const id = carrierId ?? (result.data as { id: string } | undefined)?.id
    router.push(id ? `/transport/carriers/${id}` : '/transport/carriers')
    router.refresh()
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Le code est généré automatiquement s&apos;il est laissé vide. Un transporteur n&apos;est
            pas un fournisseur : il n&apos;apparaît pas dans la liste des fournisseurs et
            n&apos;alimente aucun stock.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Code transporteur" htmlFor="code" error={errors.code?.message}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Le transport est facturé en dinars tunisiens.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <Button type="button" variant="outline" asChild>
          <Link href={carrierId ? `/transport/carriers/${carrierId}` : '/transport/carriers'}>Annuler</Link>
        </Button>
        <Button type="submit" loading={isSubmitting}>
          <Save className="h-4 w-4" />
          {carrierId ? 'Enregistrer les modifications' : 'Créer le transporteur'}
        </Button>
      </div>
    </form>
  )
